...
def paychecks(request):
    paychecks = Paycheck.objects.all().order_by("date").prefetch_related("items").prefetch_related("items__category")
    ...
    for paycheck in paychecks:
        if paycheck.date.year not in data.keys():
            data[paycheck.date.year] = {"Gross amount": {}, "Taxable amount": {}, "Net amount": {}, "Sum": [Money(0, currency)] * 12}
        ...
        data[paycheck.date.year]["Sum"][paycheck.date.month - 1] = paycheck.amount
            ...
            for category in data[year][key].keys():
                for i in range(12):
                    if data[year][key][category]["real_amount"][i] != Money(0, currency):
                        if month_sum[i] == Money(9, currency):
    ...
    return render(request, "blackbook/paychecks/list.html", {"data": data})